  ...
  for (;;) {
    ...
    case OP_INHERIT: {
      ...
      if (!IS_CLASS(superclass)) {
        runtime_error("Suprclass must be a class.");
        return INTERPRET_RUNTIME_ERROR;
      }
    ...
    case OP_METHOD:
      ...
      break;
    // TODO try to figure out if this peace of code can remain.
    default:
      puts("Undefined instruction!");
      exit(1);
    }
  }
  // we have to undefine these macros so they do not clutter the preprocessor
  // directives
#undef READ_BYTE
#undef READ_CONSTANT
#undef BINARY_OP
